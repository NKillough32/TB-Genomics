...
workflow TB_WGS {
  ...
  SAMTOOLS_QC(BWA_MEM.out.bam)
  BCFTOOLS_CALL(BWA_MEM.out.bam, reference)
  MASK_REGIONS(BCFTOOLS_CALL.out.vcf, reference, mask_bed)
  SNP_DISTS(MASK_REGIONS.out.masked_alignment)
  TBPROFILER(FASTP.out.cleaned_reads)
  PIPELINE_MANIFEST(
    SAMTOOLS_QC.out.sample_qc,
    BCFTOOLS_CALL.out.vcf,
    MASK_REGIONS.out.masked_alignment,
    SNP_DISTS.out.snp_matrix,
    TBPROFILER.out.lineage,
    TBPROFILER.out.resistance
  )
}
